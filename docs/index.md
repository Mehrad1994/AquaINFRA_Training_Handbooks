---
layout: default
title: Home
---
<div class="hero-section">
    <h1 class="hero-title">AquaINFRA Handbook</h1>
    <p class="hero-subtitle">
        A video-based training resource that helps you understand and apply AquaINFRA services and tools through guided demonstrations. It introduces the AquaINFRA infrastructure and its core components, and shows how they fit into the broader digital research ecosystem. Applied use cases then walk you through how aquatic research communities manage and reuse data in line with FAIR principles and the European Open Science Cloud (EOSC).
    </p>
</div>

<!-- Simplified Step-by-Step Path -->

<div class="homepage-section">
    <div class="section-container">

<div class="learning-path">

<!-- STEP 1 -->

<div class="path-step">
    <div class="step-marker">1</div>
    <div class="step-content">
        <h2 class="step-title">European Open Science Cloud (EOSC)</h2>
        <p class="step-desc">Understand how AquaINFRA integrates with the European Open Science Cloud infrastructure.</p>
        <div class="step-links">
            <a href="{{ relative_root }}01_eosc" class="step-link"> EOSC in Handbook →</a>
        </div>
    </div>
</div>

<!-- STEP 2 -->

<div class="path-step">
    <div class="step-marker">2</div>
    <div class="step-content">
        <h2 class="step-title">Introduction to AquaINFRA</h2>
        <p class="step-desc">Core mission and goals of the AquaINFRA project and the broader ecosystem.</p>
        <div class="step-links">
            <a href="{{ relative_root }}02_aquainfra" class="step-link"> AquaINFRA in Handbook →</a>
            <a href="https://aquainfra.eu" target="_blank" class="step-link"> Project Website →</a>
        </div>
    </div>
</div>

<!-- STEP 3 -->

<div class="path-step">
    <div class="step-marker">3</div>
    <div class="step-content">
        <h2 class="step-title">AquaINFRA Interaction Platform (AIP)</h2>
        <p class="step-desc">Discover the AquaINFRA Interaction Platform.</p>
        <div class="step-links">
            <a href="{{ relative_root }}03_aip" class="step-link"> AIP in Handbook →</a>
            <a href="https://aquainfra.dev.52north.org/" target="_blank" class="step-link">AIP Homepage →</a>
        </div>
    </div>
</div>

<!-- STEP 4 -->

<div class="path-step">
    <div class="step-marker">4</div>
    <div class="step-content">
        <h2 class="step-title">Virtual Research Environment (VRE)</h2>
        <p class="step-desc">Run your analyses in the Virtual Research Environment, with no local setup required.</p>
        <div class="step-links">
            <a href="{{ relative_root }}04_aquainfra_vre" class="step-link"> VRE in Handbook →</a>
            <a href="https://aqua.usegalaxy.eu/" target="_blank" class="step-link">AquaINFRA VRE →</a>
        </div>
    </div>
</div>

<!-- STEP 5 -->

<div class="path-step">
    <div class="step-marker">5</div>
    <div class="step-content">
        <h2 class="step-title">Data to Knowledge Package (D2KP)</h2>
        <p class="step-desc">Converting raw environmental data into actionable knowledge indicators.</p>
        <div class="step-links">
            <a href="{{ relative_root }}05_data_to_knowledge" class="step-link"> D2KP in Handbook →</a>
        </div>
    </div>
</div>

<!-- STEP 6 -->

<div class="path-step">
    <div class="step-marker">6</div>
    <div class="step-content">
        <h2 class="step-title">Applied Use Case Training</h2>
        <p class="step-desc">Work through specific training modules with dedicated code, data, and video resources.</p>
        <div class="step-links">
            <a href="{{ relative_root }}06_use_cases" class="step-link"> Explore Training Library →</a>
        </div>
    </div>
</div>

</div>

</div>
